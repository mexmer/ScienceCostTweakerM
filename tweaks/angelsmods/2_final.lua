...
if mods["angelsbioprocessing"] then
  if data.raw.item["angels-token-bio"] or data.raw.recipe["angels-token-bio"] then
    for _i, lab in pairs(data.raw["lab"]) do
      sctm.lab_input_remove(_i, "angels-token-bio")
    end
    if data.raw.item["angels-token-bio"] then
      data.raw.item["angels-token-bio"].order = "d_a[bio]-e[token-bio]"
      data.raw.item["angels-token-bio"].subgroup = "sct-bio-science-pack"
    end
